...
void SGD::updateBias(Layer* layer, const Matrix& delta, int batch_size, int epoch_num) const {
    ...
    std::vector<double> new_bias = layer->getBias() - (1 / (1 + m_decay * epoch_num)) * m_learning_rate * V_dB;
...
void Adam::updateBias(Layer* layer, const Matrix& delta, int batch_size, int epoch_num) const {
    ...
    std::vector<double> new_bias = layer->getBias() - (1 / (1 + m_decay * epoch_num)) * m_learning_rate * (V_dB_corrected / (sqrt(S_dB_corrected) + m_epsilon));
    layer->setLastBias(V_dB);
